...
- **Additional liquidity source for Liquidity Proxy**: Traders will have an alternative option to use Liquidity Proxy, which can positively impact trading outcomes.
  Reduced slippage: In XYK markets, large trades can cause significant price changes due to the trade size impacting the market price. With orderbooks, traders can place limit orders at the desired price, reducing the risk of slippage.
