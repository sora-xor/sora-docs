...
A transferência é uma transação, portanto, uma taxa de rede deve ser
paga. A taxa é calculada em XOR, portanto, mesmo que você transfira VAL,
PSWAP ou qualquer outro token, deverá ter algum XOR para cobrir a taxa.

::: informações
As tarifas para fazer transferências na SORA podem variar. Muitas vezes
são feitas propostas de governação para ajustar as taxas de transação.
A taxa real pode diferir daquela mostrada nos exemplos.
:::
...
2. Insira o endereço do destinatário e a quantidade de tokens que você gostaria de enviar
   ...
   Se você inserir um endereço que não comece com `cn`, verá uma notificação sobre a conversão do endereço. <!-- (as mentioned in the Account prefix section of [Accounts in SORA](/accounts-in-sora.md#converting-the-address-format)). -->
